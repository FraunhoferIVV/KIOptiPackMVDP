...
class Permission(BaseModel):
    ...
    duties: Optional[conlist(Duty)] = None
    target: Optional[StrictStr] = None
    __properties = ["action", "assignee", "assigner", "constraints", "duties", "target"]
    ...
    def to_dict(self):
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in duties (list)
        _items = []
        if self.duties:
            for _item in self.duties:
                if _item:
                    _items.append(_item.to_dict())
            _dict['duties'] = _items
        return _dict
    ...
    def from_dict(cls, obj: dict) -> Permission:
        ...
        _obj = Permission.parse_obj({
            ...
            "duties": [Duty.from_dict(_item) for _item in obj.get("duties")] if obj.get("duties") is not None else None,

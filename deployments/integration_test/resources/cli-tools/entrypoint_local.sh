...
function registerParticipant() {
  ...
  java -jar registration-service-cli.jar \
              -d="did:web:172.17.0.2%3A80:registration-service" \
              --http-scheme \
  ...
  local address="$4"
   ...
   do
     ...
     java -jar identity-hub-cli.jar \
                 ...
                 -c="{ $subject }" \
                 -b="$participantDid" \
                 -i="did:web:172.17.0.2%3A80:gaia-x" \
...
function seedAndRegisterParticipant() {
  ...
  local region="$2"
  local address="$3"
  local participantDid="did:web:172.17.0.2%3A80:$participantName"
...
# 'company1' 'eu' \n 'company2' 'eu' \n 'company3' 'us'
participants=$(jq -r '.include | map([.participant, .region, .address])[] | @sh' /common-resources/participants-local.json)
